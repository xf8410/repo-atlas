...
GROUPS = [
    ("🐴 赛马娘 AI 核心（这次修的就是这条线）", [
        ...
        ("umaai-rs-fanfix", "本次修复实验仓。『拉面杯溢出/松弛修复』的主战场：补丁（patches/）、九臂 A/B 测试（bench）、判词（docs/）都在这。本手册就住在这个仓库里。"),
...
idx = head('总入口') + f"""
<h1>🏠 花落仓库语义手册</h1>
<p style="color:var(--sub)">这是全部 38 个仓库、约 600 个分支的大白话说明书。<b>不写术语，只讲语义</b>——第一次玩赛马娘、第一次看代码的人也能读懂。</p>
...
<div class="callout info"><b>总判词：</b>手写评分层的单点调参已全面收敛——纯错误已修完，剩余变量的整局边际收益趋零。<b>下一步的增量不在打分层参数，在蒸馏与搜索侧</b>（先把教师修好，见 uma-ramen-teacher-lab）。</div>

<h2>为什么"方向错了还有提升"</h2>
<div class="card">
<p>提升有两种：<b>止损型</b>（填坑，方向无关）和 <b>开疆型</b>（找到更好的走法，方向有关）。</p>
<p>之前的提升全是止损型：悬崖双重收费每次罚 1149 分，修掉它任何方向都涨。v10 判词证明开疆型在手写调参里已经没有了（四个臂零变化）。真正的开疆要靠神经网络+搜索迭代（72000/76000 那个量级）。</p>
</div>
"""
